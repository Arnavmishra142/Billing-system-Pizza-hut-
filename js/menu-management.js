...
// AI FIX [2026-08-08]: The size-level toggle previously only wrote to
// settings/pizza_sizes (a standalone flag). Nothing else in the app — not the
// Menu Management "Individual Pizza Availability" list, and not the billing
// panel's own order-taking grid (js/menu.js, which only checks each item's own
// inStock field) — ever read that flag. So turning "Large" OFF looked like it
// worked (the size card showed OFF) but every individual Large pizza stayed
// orderable, forcing manual toggling of each one.
// Fix: turning a size OFF/ON now also writes inStock:false/true directly onto
// every matching pizza variant — both the products/{id}/variants/{variantId}
// subcollection doc and the denormalized variantsList array on products/{id}
// (same two writes _toggle() already does per-item, batched here across every
// affected product), or menu_items/{id} docs for the legacy architecture. The
// settings/pizza_sizes doc is still written too, purely so the size-card badge
// reflects the last bulk action.
async function _togglePizzaSize(size) {
    ...
    const nextOn = !wasOn; // target state after this toggle

    // All pizza variant items matching this size (e.g. every "* (Large)" item).
    const affected = _allItems.filter(item => _isPizzaVariant(item) && _getPizzaSize(item) === size);

    // Optimistic UI: flip the size flag AND every affected item's inStock.
    _pizzaSizes[size] = nextOn;
    const prevInStock = new Map(); // id -> previous inStock, for rollback
    affected.forEach(item => {
        prevInStock.set(item.id, item.inStock);
        item.inStock = nextOn;
    });
    ...
    try {
        ...
        if (_usingProducts) {
            // Group affected variants by their parent product so we can rebuild
            // each product's variantsList array (Firestore can't patch a single
            // array element in place — the whole array must be rewritten).
            const byProduct = new Map(); // productId -> Set of affected variant ids
            affected.forEach(item => {
                if (item._type !== 'variant' || !item._productId) return;
                if (!byProduct.has(item._productId)) byProduct.set(item._productId, new Set());
                byProduct.get(item._productId).add(item.id);
            });

            byProduct.forEach((affectedIds, productId) => {
                // Rebuild the full variantsList for this product using ALL of its
                // variants from _allItems (not just the ones matching this size),
                // so other sizes' inStock values are preserved untouched.
                const allProductVariants = _allItems.filter(i => i._type === 'variant' && i._productId === productId);
                const updatedList = allProductVariants.map(v => ({
                    id: v.id, name: (v._variantName || v.name), price: v.price || 0,
                    imageUrl: v.imageUrl || null, active: v.active !== false,
                    inStock: affectedIds.has(v.id) ? nextOn : (v.inStock !== false),
                    displayOrder: v.displayOrder || 0
                }));
                batch.update(doc(db, 'products', productId), { variantsList: updatedList });

                // Keep the variants subcollection in sync too (used when the
                // operator later edits this product in Menu Management/admin).
                affectedIds.forEach(variantId => {
                    batch.update(doc(db, 'products', productId, 'variants', variantId), { inStock: nextOn });
                });
            });

            // Edge case: a pizza size sold as a standalone product (no variants),
            // e.g. a product literally named "Cheese Pizza (Large)". Update its
            // own inStock directly since it has no variantsList to rebuild.
            affected
                .filter(item => item._type === 'product')
                .forEach(item => batch.update(doc(db, 'products', item.id), { inStock: nextOn }));
        } else {
            // Legacy architecture: each pizza variant is its own menu_items doc.
            affected.forEach(item => {
                batch.update(doc(db, 'menu_items', item.id), { inStock: nextOn });
            });
        }

        // setDoc-with-merge behavior via the batch, so the doc is created if it
        // doesn't exist yet — purely updates the size-card's own ON/OFF badge.
        batch.set(doc(db, 'settings', 'pizza_sizes'), { [size]: nextOn }, { merge: true });

        await batch.commit();
    } catch (e) {
        console.error('[menu-mgmt] pizza size toggle failed:', e);
        _pizzaSizes[size] = wasOn;
        affected.forEach(item => { item.inStock = prevInStock.get(item.id); });
        _render();
